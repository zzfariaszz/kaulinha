...
const Card = styled.div`
  ...
  border-image: ${({ $autor }) =>
    $autor === 'felipe'
      ? 'linear-gradient(90deg, #7C4D9F, #E8829A) 1'
      : 'linear-gradient(90deg, #E8829A, #FFD98E) 1'
  };
  opacity: 0;
  ...
  &::before {
    ...
    background-image: repeating-linear-gradient(
      transparent, transparent 27px, rgba(180,143,212,0.1) 28px
    );
    pointer-events: none;
    border-radius: 4px;
  }
  ...
  &:hover {
    ...
    box-shadow: 0 12px 36px rgba(61,26,94,0.14);
  }
`
...
const CardAssinatura = styled.p`
  ...
  font-size: 1.1rem;
  color: ${({ $autor }) =>
    $autor === 'felipe' ? 'var(--purple-deep)' : 'var(--pink-main)'
  };
  text-align: right;
  position: relative;
  z-index: 1;
...
export default function Mensagens() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const ref = await addDoc(collection(db, 'mensagens'), nova)
      
      // Adiciona à lista com data local formatada
      setMensagens(prev => [{ 
        id: ref.id, 
        ...nova, 
        data: new Date() 
      ...
      setForm({ titulo: '', texto: '', autor: 'felipe' })
      setFormOpen(false)
    } catch (err) {
  ...
  return (
    <Wrapper id="mensagens">
      <Inner>
        <Eyebrow>do fundo do coração</Eyebrow>
        <Title>Nossas <em>mensagens</em></Title>
        <Divider />

        <Masonry>
          {mensagens.map((m, i) => (
            <Card key={m.id} $index={i} $autor={m.autor}>
              <CardDate>{formatDate(m.data)}</CardDate>
              {m.titulo && <CardTitulo>{m.titulo}</CardTitulo>}
              <CardTexto>{m.texto}</CardTexto>
              <CardAssinatura $autor={m.autor}>
                Com amor, {m.autor === 'felipe' ? 'Felipe' : 'Kaulinha'}
              </CardAssinatura>
            </Card>
          ))}
        </Masonry>
      ...
      <FormOverlay $visible={formOpen} onClick={e => e.target === e.currentTarget && setFormOpen(false)}>
        <FormCard $visible={formOpen}>
          <FormTitle>Nova mensagem</FormTitle>
          <FormGroup>
            ...
            >
              ...
              <option value="kaulinha">Kaulinha</option>
            </FormSelect>
          </FormGroup>
          <FormGroup>
